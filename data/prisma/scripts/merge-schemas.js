...
// --- Helper Functions ---
async function readBaseSchema() {
    try {
        const content = await fs.readFile(baseSchemaPath, 'utf-8');
        console.log(`✅ Read base schema: ${path.basename(baseSchemaPath)}`);
        return content;
    }
    catch (error) {
        console.error(`❌ Error reading base schema:`, error);
        throw error;
    }
}
async function findGeneratedSchemas() {
    try {
...
async function readGeneratedSchema(filePath) {
    try {
        const content = await fs.readFile(filePath, 'utf-8');
        console.log(`✅ Read generated schema: ${path.basename(filePath)}`);
        return content;
    }
    catch (error) {
        console.error(`❌ Error reading generated schema ${filePath}:`, error);
        return '';
    }
}
...
function generateMergedSchema(baseContent, generatedContents) {
...
// ⚠️  It will be overwritten during the build process.
// =============================================================================
...
async function writeMergedSchema(content) {
    try {
        await fs.writeFile(mergedSchemaPath, content, 'utf-8');
        console.log(`✅ Generated merged schema: ${path.basename(mergedSchemaPath)}`);
    }
    catch (error) {
        console.error(`❌ Error writing merged schema:`, error);
        throw error;
    }
}
async function ensureGeneratedDirectory() {
    try {
        ...
        console.log(`✅ Ensured generated directory exists: ${generatedDir}`);
    }
    catch (error) {
...
async function mergeSchemas() {
    ...
    console.log(`🎯 Successfully merged ${generatedContents.length + 1} schema files into ${path.basename(mergedSchemaPath)}`);
}
mergeSchemas().catch(error => {
    console.error('❌ Schema merge process failed:', error);
    process.exit(1);
});
