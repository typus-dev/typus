 ...
 */
async function writeSingleSchemaFile(content, totalModels) {
 ...
 */
async function generatePrismaSchemas() {
    ...
    try {
        ...
        try {
            ...
            console.log('ℹ️  Running standalone, waiting 2 seconds for models...');
            await new Promise(resolve => setTimeout(resolve, 2000));
        }
    }
    catch (error) {
        // Fallback if DslModule not available
        console.warn('⚠️  Could not load DslModule, using fallback timeout (2s)');
        await new Promise(resolve => setTimeout(resolve, 2000));
    }
 ...
 */
function extractManualModels(content, dslModelNames) {
    const lines = content.split('\n');
    const modelBlocks = [];
    let currentBlock = [];
    let inBlock = false;
...
// --- Main execution ---
generatePrismaSchemas().catch((error) => {
    console.error('❌ Prisma schema generation failed:', error);
    process.exit(1);
});
//# sourceMappingURL=generate-prisma-schemas.js.map